...
class KnittingCounterWidget : AppWidgetProvider() {
    ...
    private fun updateCounterValue(context: Context, appWidgetId: Int, isStitch: Boolean, change: Int) {
        ...
        if (isStitch) {
            counter.stitches = (counter.stitches + change).coerceAtLeast(0)
        } else {
            counter.rows = (counter.rows + change).coerceAtLeast(0)
        }
    ...
    private fun parseCounterId(id: String): Pair<Int, Int>? = id.split("-").takeIf { it.size == 2 }?.let {
        it[0].toIntOrNull()?.let { proj -> it[1].toIntOrNull()?.let { ctr -> Pair(proj, ctr) } }
    }
    ...
    companion object {
        ...
        fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, vararg appWidgetIds: Int) {
            appWidgetIds.forEach { appWidgetId ->
                ...
                if (counterId.isNotEmpty()) {
                    ...
                    val (projectIndex, counterIndex) = counterId.split("-").mapNotNull { it.toIntOrNull() }.takeIf { it.size == 2 }?.let { Pair(it[0], it[1]) } ?: Pair(-1, -1)
                ...
                val addCounterPendingIntent = PendingIntent.getBroadcast(
                    context, appWidgetId * 10 + 5, addCounterIntent,
                    PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
                )
